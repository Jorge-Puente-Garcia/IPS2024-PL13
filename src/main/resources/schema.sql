...
CREATE TABLE Producto (id INTEGER PRIMARY KEY AUTOINCREMENT,nombre TEXT NOT NULL,referencia TEXT NOT NULL UNIQUE, datosBasicos TEXT NOT NULL, precioPersona INTEGER NOT NULL, precioEmpresa INTEGER NOT NULL, iva DECIMAL(3,2) NOT NULL, unidades INTEGER NOT NULL, localizacion_id INTEGER,id_categoria INTEGER NOT NULL, unidadesMinimas INTEGER , unidadesReposicion INTEGER , FOREIGN KEY (localizacion_id) REFERENCES Localizacion(id));
...
CREATE TABLE Carrito (id INTEGER PRIMARY KEY AUTOINCREMENT, dni TEXT NOT NULL, referencia TEXT NOT NULL, cantidad INTEGER NOT NULL, precioBase INTEGER NOT NULL, iva DECIMAL(3,2) NOT NULL, precio INTEGER NOT NULL, FOREIGN KEY (referencia) REFERENCES Producto(referencia),FOREIGN KEY (dni) REFERENCES Cliente(dni));
